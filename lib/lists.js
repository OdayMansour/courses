...
module.exports.addToList_get = function (req, res) {
    res.send(
`<html>
  <body>
    <form action="/api/lists/addtolist/" method="post">
...
function returnList(rows, res_req) {
    var res = res_req[0]
    var req = res_req[1]
...
function linkItemToList(rows, res_req) {
    var res = res_req[0]
    var req = res_req[1]
...
function addExistingItemToList(rows, res_req) {
    linkItemToList(rows, res_req)
}

function addNewItemToList(res_req) {
    var res = res_req[0]
    var req = res_req[1]
...
function checkItem(rows, res_req) {
    var res = res_req[0]
    var req = res_req[1]

    if (rows.length == 1) { // Item exists
        addExistingItemToList(rows, res_req)
    } else { // Item does not exist
        addNewItemToList(res_req)
    }
}

function checkList(rows, res_req) {
    var res = res_req[0]
    var req = res_req[1]
...
module.exports.addToList_post = function (req, res) {
